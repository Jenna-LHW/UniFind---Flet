...
def report_found_view(page: ft.Page, go):
    ...
    found_at    = ft.TextField(label='Found At Location *', prefix_icon=ft.Icons.LOCATION_ON)
    date_found  = ft.TextField(label='Date Found * (YYYY-MM-DD)', prefix_icon=ft.Icons.CALENDAR_TODAY)
    ...
    def do_submit(e):
        ...
        if not all([item_name.value, category.value, description.value, found_at.value, date_found.value]):
        ...
        status, resp = report_found_item(data)
        loading.visible = False

        if status == 201:
            success.value = 'Found item reported successfully!'
            item_name.value = category.value = description.value = found_at.value = date_found.value = ''
        else:
    ...
    return ft.Column([
        ft.Container(
            content=ft.Row([
                ...
                ft.Text('Report a Found Item', size=18, weight=ft.FontWeight.BOLD, color='#2c2c2a'),
            ], spacing=10),
            padding=20,
            bgcolor='white',
            border_radius=12,
        ...
        ft.Container(
            content=ft.Column([
                ft.Text('Item Details', size=14, weight=ft.FontWeight.W_600, color='#3b5c38'),
                item_name, category, description,
                ft.Divider(height=8, color='transparent'),
                ft.Text('Location & Date', size=14, weight=ft.FontWeight.W_600, color='#3b5c38'),
                found_at, date_found,
                error, success,
                ft.Row([
                    loading,
                    ft.ElevatedButton('Submit Report', bgcolor='#3b5c38', color='white',
