...
def _field(label, icon, multiline=False, min_lines=1, max_lines=1, hint=None):
    return ft.TextField(
        ...
        min_lines=min_lines, max_lines=max_lines, hint_text=hint,
        border_radius=12, filled=True, bgcolor='white',
        border_color='#d6d1c8', focused_border_color=GREEN,
...
def report_found_view(page: ft.Page, go):
    ...
    found_at    = _field('Found At Location *', ft.Icons.LOCATION_ON_OUTLINED)
    date_found  = _field('Date Found * (YYYY-MM-DD)', ft.Icons.CALENDAR_TODAY_OUTLINED)
    ...
    loading = ft.ProgressRing(visible=False, width=22, height=22, color=GREEN)
    ...
    def do_submit(e):
        ...
        data = {
            ...
            'description': description.value.strip(), 'found_at': found_at.value.strip(),
        ...
        if status == 201:
            ...
            for f in [item_name, description, found_at, date_found, photo_path]:
                f.value = ''
            category.value = None
        else:
            ...
            content=ft.Column([
                ...
                ft.Container(
                    content=ft.Row([
                        ...
                        ft.Text('Enter the full file path to your photo.', size=11, color='#9a8f80'),
                    ], spacing=6),
                ),
                photo_path,
                ft.Container(height=4),
